...
<!-- for haemorrhagic stroke deep recent (id='6') and  haemorrhagic stroke deep old (id='7') we need to be careful about intra-cerebral which is loc-deep. If it cooccurs with a loc-cortical then we need to ignore it -->
<xsl:template match="l[@id='6']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:choose>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[not(@neg) and (@type='haemorrhagic_stroke' and @loc-deep='true' and not(@loc-cortical) and @time-recent='true')]">
        <xsl:attribute name="choice">pos</xsl:attribute>
      </xsl:when>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[not(.~'intra.*cerebral') and not(@neg) and @type='haemorrhagic_stroke' and @loc-deep='true' and @loc-cortical='true' and @time-recent='true']">
        <xsl:attribute name="choice">pos</xsl:attribute>
      </xsl:when>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[.~'intra.*cerebral' and not(@neg) and @type='haemorrhagic_stroke' and @loc-deep='true' and @loc-cortical='true' and @time-recent='true']">
      </xsl:when>
    </xsl:choose>
    <xsl:apply-templates select="node()"/>
  </xsl:copy>
...
<xsl:template match="l[@id='7']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:choose>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[not(@neg) and (@type='haemorrhagic_stroke' and @loc-deep='true' and not(@loc-cortical) and @time-old='true')]">
        <xsl:attribute name="choice">pos</xsl:attribute>
      </xsl:when>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[not(.~'intra.*cerebral') and not(@neg) and @type='haemorrhagic_stroke' and @loc-deep='true' and @loc-cortical='true' and @time-old='true']">
        <xsl:attribute name="choice">pos</xsl:attribute>
      </xsl:when>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[.~'intra.*cerebral' and not(@neg) and @type='haemorrhagic_stroke' and @loc-deep='true' and @loc-cortical='true' and @time-old='true']">
      </xsl:when>
    </xsl:choose>
    <xsl:apply-templates select="node()"/>
  </xsl:copy>
...
<xsl:template match="l[@id='10']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:choose>
      <xsl:when test="ancestor::document//s[@proc='yes']//ng[not(ancestor::ng) and @neg and @type='haemorrhagic_stroke' and not(@*[name()~'^loc'] and @*[name()~'^time']) and not(ancestor::document//s[@proc='yes']//ng[not(ancestor::ng) and not(@neg) and @type='haemorrhagic_stroke' and (@*[name()~'^loc'] or @*[name()~'^time'])])]">
      </xsl:when>
