	...
	<xsl:template match="/">
		<HTML>
			...
			<BODY>
				...
				<H2>NER Score Summary (collapsed types)</H2>
				...
				<TABLE>
					...
					<xsl:variable name="breakdowns">
						<xsl:for-each
							select="performance/run/document/NERCollapsed/breakdown[not(@isScored)]">
							<xsl:sort select="@type" />
							<xsl:copy-of select="." />
						</xsl:for-each>
					</xsl:variable>
					...
					<xsl:for-each select="performance/run">
						<xsl:call-template name="output-scores">
							<xsl:with-param name="title"
								select="'TOTAL'" />
							<xsl:with-param name="tp"
								select="sum(document/NERCollapsed/@tp)" />
							<xsl:with-param name="fp"
								select="sum(document/NERCollapsed/@fp)" />
							<xsl:with-param name="fn"
								select="sum(document/NERCollapsed/@fn)" />
							<xsl:with-param name="alpha"
								select="$alphaValue" />
						</xsl:call-template>
					</xsl:for-each>
				...
				<H2>Term Normalisation Score Summary</H2>
...
<!-- ////////////////////////// -->
					<xsl:variable name="breakdownsTI">
						<xsl:for-each
							select="performance/run/document/TI/breakdown[not(@isScored)]">
							<xsl:sort select="@type" />
							<xsl:copy-of select="." />
						</xsl:for-each>
					</xsl:variable>
					...
					<xsl:for-each
						select="exslt:node-set($breakdownsTI)/breakdown">
						<xsl:variable name="pos" select="position()" />
						<xsl:if
							test="$pos = 1 or @type != current()/preceding-sibling::breakdown[1]/@type">
							<xsl:variable name="subset"
								select="exslt:node-set($breakdownsTI)/breakdown[@type = current()/@type]" />
							<xsl:call-template name="output-scores">
								<xsl:with-param name="title"
									select="@type" />
								<xsl:with-param name="tp"
...
<!-- ////////////////////////// -->
				...
				<H2>Relation Extraction Score Summary (ignoring entity types)</H2>
				...
				<TABLE>
					...
					<xsl:variable name="breakdowns">
						<xsl:for-each
							select="performance/run/document/RECollapsed/breakdown[not(@isScored)]">
							<xsl:sort select="@type" />
							<xsl:copy-of select="." />
						</xsl:for-each>
					</xsl:variable>
					...
					<xsl:for-each select="performance/run">
						<xsl:call-template name="output-scores">
							<xsl:with-param name="title"
								select="'TOTAL'" />
							<xsl:with-param name="tp"
								select="sum(document/RECollapsed/@tp)" />
							<xsl:with-param name="fp"
								select="sum(document/RECollapsed/@fp)" />
							<xsl:with-param name="fn"
								select="sum(document/RECollapsed/@fn)" />
							<xsl:with-param name="alpha"
									select="$alphaValue" />
						</xsl:call-template>
					</xsl:for-each>
				...
				<H2>Relation Attribute Score Summary</H2>
				...
				<H2>Relation Property Score Summary</H2>
	...
	<xsl:template name="output-scores">
		...
		<TR>
			...
			<TD>
				<xsl:value-of
					select="format-number($tp + $fp + $fn, '#,##0')" />
			</TD>
			<TD>
			...
			<TD>
				<xsl:value-of
					select="format-number($precision, '0.00')" />
			</TD>
			<TD>
