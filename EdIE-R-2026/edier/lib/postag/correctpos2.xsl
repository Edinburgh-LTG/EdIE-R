...
<xsl:template match="w[.~'^([Nn]il|NIL|[Pp]lease|PLEASE)$']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
...
<xsl:template match="w[.~'^([Rr]ule)$' and following-sibling::w[1][.='out']]">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">VB</xsl:attribute>
    <xsl:apply-templates select="node()"/>
  </xsl:copy>
</xsl:template>

<xsl:template match="w[.='isno']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">VBZ</xsl:attribute>
    <xsl:apply-templates select="node()"/>
  </xsl:copy>
</xsl:template>

<xsl:template match="w[.='out' and preceding-sibling::w[1][.~'^([Rr]ule)$']]">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">RP</xsl:attribute>
    <xsl:apply-templates select="node()"/>
  </xsl:copy>
</xsl:template>
...
<xsl:template match="w[.~'^[Bb]leed$' and @p~'V' and not(preceding-sibling::w[1][.='to'])]">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">NN</xsl:attribute>
    <xsl:apply-templates select="node()"/>
...
<xsl:template match="w[.~'^([Ss]toke|[Nn]ucleus|[Hh]aemosiderin|[Ii]nfarct|hemisphere)$' and @p~'V']">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">NN</xsl:attribute>
    <xsl:apply-templates select="node()"/>
...
<xsl:template match="w[.='established' and preceding-sibling::w[1][.~'^[Rr]ecently']]">
  <xsl:copy>
    <xsl:apply-templates select="@*"/>
    <xsl:attribute name="p">JJ</xsl:attribute>
    <xsl:attribute name="g">JJ</xsl:attribute>
